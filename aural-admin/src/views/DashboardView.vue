...
const backupAgeText = (record) => {
  if (!record?.createdAt) return '暂无备份记录'
  const ageHours = Math.max(0, Math.round((Date.now() - new Date(record.createdAt).getTime()) / 3600000))
  if (ageHours < 24) return `${ageHours || 1} 小时前 / ${record.status || '-'}`
  return `${Math.round(ageHours / 24)} 天前 / ${record.status || '-'}`
}

const diskSummaryText = (disk) => {
...
const monitorTypeLabel = (type) => ({
  frontend_error: 'JS',
  resource_error: '资源',
  api_error: 'API',
  form_submit_failed: '表单'
}[type] || type || '监控')
